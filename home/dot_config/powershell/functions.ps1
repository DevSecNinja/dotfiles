...
function Test-WingetUpdates {
    ...
    if ($UseWingetModule -and $wingetModule) {
        try {
            ...
            if ($updates -and $updates.Count -gt 0) {
                Write-Host "✅ Found $($updates.Count) package update(s) available" -ForegroundColor Green
                return $true
            }
            else {
    ...
    if (-not $UseWingetModule -or -not $wingetModule) {
        ...
        elseif ($result -match "(\d+) upgrades available") {
            $count = $Matches[1]
            Write-Host "✅ Found $count package update(s) available" -ForegroundColor Green
            return $true
        }
        else {
...
function Invoke-WingetUpgrade {
    ...
    if (-not $Force) {
        Write-Host "`n📋 Detection Phase: Checking for updates..." -ForegroundColor Yellow
        $hasUpdates = Test-WingetUpdates -UseWingetModule $UseWingetModule

        if (-not $hasUpdates) {
            Write-Host "`n✅ No updates available. Skipping upgrade." -ForegroundColor Green
            return
        }
    }
    else {
        Write-Host "`n⚠️  Skipping detection phase (Force mode)" -ForegroundColor Yellow
    }
    ...
    if (-not $UseWingetModule -or -not $wingetModule) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "`n❌ Upgrade completed with errors (exit code: $LASTEXITCODE)" -ForegroundColor Red
        }
    }
