...
BeforeAll {
    ...
    $scriptPath = Join-Path $repoRoot "home/dot_config/powershell/scripts/New-SigningCert.ps1.tmpl"

    # If running in CI/actual environment, might need to process template
    if (-not (Test-Path $scriptPath)) {
        $scriptPath = Join-Path $repoRoot "home/dot_config/powershell/scripts/New-SigningCert.ps1"
    }
