...
@test "test-fish-config: package-managed mise upgrades with brew before activation" {
	if ! command -v fish >/dev/null 2>&1; then
		skip "Fish not installed"
	fi
...
case "$1 $2" in
	"activate fish")
		...
		;;
	"self-update -y")
		printf '%s\n' 'mise ERROR mise is installed via a package manager, cannot update' >&2
		exit 1
		;;
	"completion fish")
		exit 0
...
EOF
	...
	export HOME="$BATS_TEST_TMPDIR/home"
	export TERM_PROGRAM="vscode"
	export MISE_FAKE_MARKER="$BATS_TEST_TMPDIR/mise-updated-by-brew"
